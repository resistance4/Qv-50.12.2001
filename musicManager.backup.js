...
class MusicManager {
    ...
    async joinVoiceChannel(member, retryCount = 0) {
        ...
        try {
            ...
            if (error.message?.includes('socket') || error.message?.includes('ECONNREFUSED') || error.message?.includes('ip discovery')) {
                console.log(`⚠️ UDP/Socket error detected, retrying in 2 seconds...`);
                ...
                if (retryCount < 1) {
                    ...
                    throw new Error(`Failed to establish voice connection after ${retryCount + 1} attempts. This may be due to network/UDP issues.`);
                }
            }
    ...
    async playNext(guildId) {
        ...
        try {
            ...
            try {
                // Create stream with improved options that work reliably
                ...
                // Ensure stream is valid and handle all errors
                audioStream.on('error', (error) => {
                    console.error(`❌ Stream error: ${error.message}`);
                    // Socket errors are handled by player error handler
                });
            ...
            resource.on('error', (error) => {
                ...
                if (error.message?.includes('socket') || error.message?.includes('ECONNREFUSED')) {
                    console.log(`⚠️ UDP/Socket error during playback`);
                }
            });
