...
class TicketManager {
    ...
    async createTicketPanel(message, channelId, panelMessage, roleId = null) {
        try {
            ...
            // Parse panel message format: "title | message | image_url" or "title | message"
            // Image positioning: append "top" or "bottom" to URL (default: bottom)
            let embedTitle = '🎫 Support Ticket System';
            let embedDescription = 'Click the button below to open a support ticket.\n\nOur staff team will assist you shortly!';
            let imageUrl = null;
            let imagePosition = 'bottom'; // 'top' (thumbnail) or 'bottom' (image)

            if (panelMessage) {
                const parts = panelMessage.split('|').map(p => p.trim());
                
                if (parts.length >= 1 && parts[0]) {
                    embedTitle = parts[0];
                }
                if (parts.length >= 2 && parts[1]) {
                    embedDescription = parts[1];
                }
                if (parts.length >= 3 && parts[2]) {
                    const imagePart = parts[2].toLowerCase();
                    // Check for position keywords at the end
                    if (imagePart.endsWith(' top')) {
                        imageUrl = parts[2].slice(0, -4).trim();
                        imagePosition = 'top';
                    } else if (imagePart.endsWith(' bottom')) {
                        imageUrl = parts[2].slice(0, -7).trim();
            ...
            if (imageUrl) {
                if (imagePosition === 'top') {
                    // Use thumbnail for top-right position
                    embed.setThumbnail(imageUrl);
                } else {
                    // Use image for bottom position
            ...
            // Send acknowledgement using acknowledgement service
            const ackMessage = `Ticket panel created in ${channel}\n**Message ID:** ${panelMsg.id}\n**Role to Ping:** ${roleId ? `<@&${roleId}>` : 'None'}`;
            await this.ackService.send(message, ackMessage, 'ticket');

            // Create log embed for audit trail
