...
class RoleManager {
    ...
    async createRole(message, args) {
        ...
        try {
            ...
            await this.ackService.send(
                message,
                `Role created: ${newRole}`,
                'role'
            );

            const logEmbed = this.createLogEmbed(message.author, `Role created: ${newRole.name} (ID: ${newRole.id})`);
    ...
    async deleteRole(message, args) {
        ...
        try {
            ...
            const logEmbed = this.createLogEmbed(message.author, `Role deleted: ${roleName} (ID: ${roleId}, ${memberCount} members)`);
            await this.sendLogMessage(message.guild, logEmbed);
    ...
    async editRole(message, args) {
        ...
        if (editType === 'name') {
            ...
            try {
                ...
                const logEmbed = this.createLogEmbed(message.author, `Role name updated: ${oldName} → ${newName}`);
                await this.sendLogMessage(message.guild, logEmbed);
                return;
            } catch (error) {
        ...
        if (editType === 'color') {
            ...
            try {
                ...
                const logEmbed = this.createLogEmbed(message.author, `Role color updated: ${role.name} → ${colorValue}`);
                await this.sendLogMessage(message.guild, logEmbed);
                return;
            } catch (error) {
        ...
        try {
            ...
            await this.ackService.send(
                message,
                `**Role Permissions Updated**\n\n**Role:** ${role}\n**Permission Type:** ${permissionDescription}`,
                'role'
            );

            const logEmbed = this.createLogEmbed(message.author, `Role permissions updated: ${role.name} (${permissionDescription})`);
            await this.sendLogMessage(message.guild, logEmbed);
    ...
    async removeAllRoles(message, args) {
        ...
        try {
            ...
            const embed = this.createLogEmbed(message.author, `All roles removed from <@${targetUser.id}>\n**Roles Removed:** ${roleCount}`);
    ...
    async addRole(message, args) {
        ...
        try {
            ...
            await this.ackService.send(
                message,
                `Role ${addRole} added to <@${addRoleUser.id}>`
            );

            const logEmbed = this.createLogEmbed(message.author, `Role added: ${addRole.name} to ${addRoleUser.username} (${addRoleUser.id})`);
            await this.sendLogMessage(message.guild, logEmbed);
        } catch (error) {
    ...
    async removeRole(message, args) {
        ...
        try {
            ...
            await this.ackService.send(
                message,
                `Role removed: ${removeRole} from <@${removeRoleUser.id}>`
            );

            const logEmbed = this.createLogEmbed(message.author, `Role removed: ${removeRole.name} from ${removeRoleUser.username} (${removeRoleUser.id})`);
            await this.sendLogMessage(message.guild, logEmbed);
        } catch (error) {
    ...
    async handleAddRoleSlash(interaction) {
        ...
        try {
            ...
            await this.ackService.send(
                interaction,
                `Role ${role} added to <@${user.id}>`,
                'role'
            );

            const logEmbed = this.createLogEmbed(interaction.user, `Role added: ${role.name} to ${user.username} (${user.id})`);
    ...
    async handleRemoveRoleSlash(interaction) {
        ...
        try {
            ...
            await this.ackService.send(
                interaction,
                `Role removed: ${role} from <@${user.id}>`,
                'role'
            );

            const logEmbed = this.createLogEmbed(interaction.user, `Role removed: ${role.name} from ${user.username} (${user.id})`);
            await this.sendLogMessage(interaction.guild, logEmbed);
        } catch (error) {
    ...
    async handleCreateRoleSlash(interaction) {
        ...
        try {
            ...
            await this.ackService.send(
                interaction,
                `**Role Created**\n\n**Role:** ${newRole}\n**Role ID:** \`${newRole.id}\`${color ? `\n**Color:** ${color}` : ''}`,
                'role'
            );

            const logEmbed = this.createLogEmbed(interaction.user, `Role created: ${newRole.name} (ID: ${newRole.id})`);
            await this.sendLogMessage(interaction.guild, logEmbed);
        } catch (error) {
        ...
        try {
            ...
            switch(property) {
                case 'name':
                    ...
                    updateMessage = `**Property:** Name\n**Old Name:** ${oldName}\n**New Name:** ${value}`;
                    logMessage = `Role name updated: ${oldName} → ${value}`;
                    break;
                case 'color':
                    if (!value) return await interaction.reply({ content: '❌ Value required for color', ephemeral: true });
                    await role.setColor(value);
                    updateMessage = `**Property:** Color\n**New Color:** ${value}`;
                    logMessage = `Role color updated: ${role.name} → ${value}`;
                    break;
                case 'admin':
                    await role.setPermissions([PermissionFlagsBits.Administrator]);
                    updateMessage = `**Property:** Permissions\n**Permission Type:** Administrator (Full Control)`;
                    logMessage = `Role permissions updated: ${role.name} (Administrator)`;
                    break;
                case 'text':
                    ...
                    updateMessage = `**Property:** Permissions\n**Permission Type:** Text Channel Permissions`;
                    logMessage = `Role permissions updated: ${role.name} (Text Permissions)`;
                    break;
                case 'voice':
                    await role.setPermissions([
                        PermissionFlagsBits.ViewChannel,
                    ...
                    updateMessage = `**Property:** Permissions\n**Permission Type:** Voice Channel Permissions`;
                    logMessage = `Role permissions updated: ${role.name} (Voice Permissions)`;
                    break;
            }
            ...
            const logEmbed = this.createLogEmbed(interaction.user, logMessage);
            await this.sendLogMessage(interaction.guild, logEmbed);
        } catch (error) {
