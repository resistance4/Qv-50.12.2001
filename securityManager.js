...
class SecurityManager {
    ...
    async punishViolator(guild, user, violationType, violationCount) {
        ...
        try {
            ...
            const punishEmbed = this.createLogEmbed(user, `Anti-Nuke Violation - User Punished\n**Violation Type:** ${violationType}\n**Violation Count:** ${violationCount}\n**Punishment:** ${settings.punishmentType.toUpperCase()}\n**Status:** ✅ THREAT NEUTRALIZED`);
    ...
    async monitorRoleDelete(role) {
        ...
        try {
            ...
            const embed = this.createLogEmbed(logEntry.executor, `Anti-Nuke: Role Deletion Blocked & Restored\n**Role Name:** ${role.name}\n**Violations:** ${violationCount}/${settings.maxViolations}`);
    ...
    async monitorRoleUpdate(oldRole, newRole) {
        ...
        try {
            ...
            const embed = this.createLogEmbed(logEntry.executor, `Anti-Nuke: Role Update Blocked & Reverted\n**Role:** ${newRole.toString()}\n**Violations:** ${violationCount}/${settings.maxViolations}`);
    ...
    async enableAllSecurity(message) {
        ...
        const activationMessage = `✅ All security features enabled\n**Status:** Full Protection Active\n**Punishment:** ${settings.punishmentType.toUpperCase()}`;

        await this.ackService.send(message, activationMessage);
        
        const logEmbed = this.createLogEmbed(message.author, activationMessage);
        await this.sendSecurityLog(message.guild, logEmbed);
    }
    ...
    async disableAllSecurity(message) {
        ...
        const deactivationMessage = `❌ All security features disabled\n**Status:** Protection Disabled\n**Warning:** Server is vulnerable to attacks`;

        await this.ackService.send(message, deactivationMessage);
        
        const logEmbed = this.createLogEmbed(message.author, deactivationMessage);
        await this.sendSecurityLog(message.guild, logEmbed);
    }
    ...
    async showSecurityStatus(message) {
        ...
        let statusMessage;
        if (settings.enabled) {
            statusMessage = `✅ **${serverName}** is secured\n**Punishment:** ${settings.punishmentType.toUpperCase()}\n**Max Violations:** ${settings.maxViolations}`;
        } else {
            statusMessage = `⚠️ **${serverName}** protection disabled\n**Enable security** to protect your server`;
        }

        await this.ackService.send(message, statusMessage);
    }
