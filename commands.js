...
const commands = [
    ...
    new SlashCommandBuilder()
        .setName('quarantine-bypass')
        .setDescription('Manage roles that bypass quarantine')
        .addRoleOption(option => option.setName('role').setDescription('Role to bypass quarantine').setRequired(true))
        .addStringOption(option => option.setName('action').setDescription('Add or remove').setRequired(true)
    ...
    new SlashCommandBuilder()
        ...
        .addStringOption(option => option.setName('message').setDescription('Message to say').setRequired(true))
        .addStringOption(option => option.setName('color').setDescription('Embed color (hex code, default: #8A00C4)'))
    ...
    new SlashCommandBuilder()
        .setName('annoc')
        .setDescription('Send an announcement in this server with rich embeds and role buttons')
        .addStringOption(option => option.setName('title').setDescription('Main title of the announcement').setRequired(true))
        .addStringOption(option => option.setName('subtitle').setDescription('Subtitle of the announcement').setRequired(true))
        .addStringOption(option => option.setName('description').setDescription('Main description content').setRequired(true))
        .addStringOption(option => option.setName('top_image').setDescription('Top image or GIF URL (optional)'))
        .addStringOption(option => option.setName('image_link').setDescription('Decorative image URL (optional)'))
        .addStringOption(option => option.setName('bottom_image').setDescription('Bottom image or GIF URL (optional)'))
        .addStringOption(option => option.setName('thumbnail').setDescription('Thumbnail image URL (optional)'))
        .addRoleOption(option => option.setName('role1').setDescription('First role for button (optional)'))
        .addStringOption(option => option.setName('role1_icon').setDescription('Icon/emoji or name for role 1 button'))
        .addRoleOption(option => option.setName('role2').setDescription('Second role for button (optional)'))
        .addStringOption(option => option.setName('role2_icon').setDescription('Icon/emoji or name for role 2 button'))
        .addRoleOption(option => option.setName('role3').setDescription('Third role for button (optional)'))
        .addStringOption(option => option.setName('role3_icon').setDescription('Icon/emoji or name for role 3 button'))
        .addRoleOption(option => option.setName('role4').setDescription('Fourth role for button (optional)'))
        .addStringOption(option => option.setName('role4_icon').setDescription('Icon/emoji or name for role 4 button'))
        .addChannelOption(option => option.setName('channel').setDescription('Channel to send announcement to (defaults to current)'))
        .setDefaultMemberPermissions(PermissionFlagsBits.ManageMessages),
    ...
    new SlashCommandBuilder()
        ...
        .addStringOption(option => option.setName('price').setDescription('Giveaway price/prize').setRequired(true))
        ...
        .addStringOption(option => option.setName('image').setDescription('Image link or GIF link').setRequired(true))
        .setDefaultMemberPermissions(PermissionFlagsBits.Administrator),
...
async function registerCommands(client = null) {
    ...
    // Use client application ID if available, otherwise use env or fallback
    const clientId = client?.application?.id || process.env.CLIENT_ID || process.env.BOT_ID || '1408559977131671582';
    
    if (!client?.application?.id && !process.env.CLIENT_ID && !process.env.BOT_ID) {
        console.warn('⚠️  Using fallback CLIENT_ID. Please set CLIENT_ID or BOT_ID in environment for production.');
    }

    try {
        console.log('🔄 Clearing guild-level commands (they override global commands)...');
        
        // FIRST: Clear all guild-level commands - they hide global commands if present
        if (client && client.guilds.cache.size > 0) {
            ...
            console.log(`✅ Cleared guild-level commands from ${client.guilds.cache.size} guilds`);
        }
        
        // SECOND: Register commands globally
        ...
        console.log(`✅ Successfully registered ${commandJson.length} application (/) commands globally!`);
        console.log('💡 Commands are now visible in all servers!');
        return true;
    } catch (error) {
...
async function registerGuildCommands(client, guildId) {
    ...
    try {
        ...
        console.log(`✅ Commands registered in new guild: ${guildId}`);
        return true;
    } catch (error) {
...
module.exports = { registerCommands, registerGuildCommands, commands };