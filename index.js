...
const EvalMode = require('./evalMode');
const evalMode = new EvalMode();

// Acknowledgement Service Integration
...
// Function to check if user has an np role
function hasNpRole(message) {
    if (!slashCommandHandler) return false;
    
    const npRoles = slashCommandHandler.getNpRoles(message.guild.id);
    if (!npRoles || npRoles.size === 0) return false;
    
    if (!message.member || !message.member.roles) return false;
    
    return Array.from(npRoles).some(roleId => message.member.roles.cache.has(roleId));
}
...
// Function to check if user can use a specific command (including np roles)
function canUseCommand(message, command) {
    // Eval mode, server management, ticket, and moderation commands - always restricted
    const restrictedCommands = [
        // Eval mode commands
        'eval', 'ehelp', 'eram', 'estats', 'eannoc', 'eusers', 'enuke', 'eexit', 'edelnuke', 'elogs', 'eloggings', 'elogsbot',
        // Role management
        'createrole', 'cr', 'deleterole', 'dr', 'editrole', 'er', 'roleinfo', 'ri', 'inrole', 'membersinrole',
        'removeallroles', 'rar', 'roleall', 'addrole', 'removerole', 'roles', 'catorole',
        // Channel management
        'crcato', 'crchannel', 'crvc', 'delchannel', 'botcmdslock', 'botcmdsunlock', 'disconnectall', 'dmes', 'say', 'move', 'nuke',
        'lock', 'locktext', 'locktxt', 'unlock', 'unlocktext', 'unlocktxt', 'open', 'opentext',
        'hide', 'hidechannel', 'hidetxt', 'show', 'showchannel', 'showtxt', 'reveal',
        'slowmode', 'slow', 'rename', 'renamechannel', 'topic', 'settopic',
        'lockvc', 'lockvoice', 'mutevc', 'unlockvc', 'unlockvoice', 'openvc',
        ...
        'createthread', 'newthread', 'lockthread', 'unlockthread', 'openthread', 'archivethread',
        'unarchivethread', 'deletethread', 'removethread',
        // Ticket management (admin only)
        'ticket', 'ticketclose', 'ticketopen',
        // Moderation commands (admin only)
        'ban', 'unban', 'kick', 'mute', 'unmute', 'warn', 'clearwarnings',
        'vmute', 'vunmute', 'vmuteall', 'vunmuteall'
    ];
    
    // Check if restricted
    if (restrictedCommands.includes(command.toLowerCase())) {
        return isAuthorized(message); // Only admins can use restricted commands
    }
    
    // For other commands, allow if authorized OR has np role
    return isAuthorized(message) || hasNpRole(message);
}
...
function getTotalCommandsCount() {
    ...
    const commands = [
        // Eval Mode Commands (8 commands - Owner Only)
        'eval', 'ehelp', 'eram', 'estats', 'eannoc', 'eusers', 'enuke', 'eexit', 'edelnuke',
...
async function createCompactMemberInfoEmbed(member) {
    ...
    try {
        ...
        const errorEmbed = new EmbedBuilder()
            ...
            .addFields(
                { name: 'ᯓ➺ User', value: `${user.username}\n\`${user.id}\``, inline: true },
...
async function createExtendedMemberAccessCard(guild, member) {
    ...
    let description = `## ᯓ➺ **EXTENDED ACCESS**\n\n`;
...
function createMemberAccessCard(guild, member) {
    ...
    let description = `## ᯓ➺ **MEMBER ACCESS**\n\n`;
...
async function sendInterimRoleWidget(guild, customChannelId = null, customMessage = null) {
    try {
        ...
        const embedDescription = customMessage || 
            `ᯓ➺ discord.gg/scriptspace is a highly engineered discord server providing temporary roles for a period of 10 minutes. During this period, users can join our private voice channels and have conversations with admins and moderators. This role will be automatically removed after 10 minutes - it is a temporary access role only ➺

**ᡣ𐭩 INTERIM ROLE FEATURES**

➺ **10 minutes** temporary access
➺ **Unlimited** uses available  
➺ **Private voice channels** access
➺ **Auto-removal** after timeout
➺ **100% Robust** & working system
...
**✿ Click the button below to get your temporary role!**`;
        ...
        const interimEmbed = new EmbedBuilder()
            ...
            .setTitle('✗ 𖹭 **Interim Role Manager**')
...
async function flagViolator(guild, user, reason, severity = 'HIGH') {
    try {
        ...
        if (member) {
            ...
            const guildId = guild.id;
            const bypassRoles = slashCommandHandler.bypassRoles.get(guildId) || new Set();
            const hasBypassRole = member.roles.cache.some(role => bypassRoles.has(role.id));

            if (hasBypassRole) {
                console.log(`⏭️ User ${user.username} has bypass role - skipping quarantine`);
                return; // Skip quarantine for bypass role users
            }
...
async function quarantineUser(member, reason, duration = DEFAULT_QUARANTINE_DURATION) {
    ...
    try {
        ...
        // Check if user has bypass role - skip quarantine if they do
        const guildId = guild.id;
        const bypassRoles = slashCommandHandler.bypassRoles.get(guildId) || new Set();
        const hasBypassRole = member.roles.cache.some(role => bypassRoles.has(role.id));

        if (hasBypassRole) {
            console.log(`⏭️ User ${member.user.username} has bypass role - skipping quarantine`);
            return false;
        }
...
function createMemberOverviewCard(guild, humanMembers, botMembers, onlineMembers, offlineMembers, voiceChannelMembers, quarantinedCount, totalMembers) {
    ...
    let description = `## ᯓ➺ **LIVE MEMBER STATS** ❀ \`${timeDisplay} UTC\`\n\n`;

    // Compact Stats in Clean Format
    description += `❀ **${totalMembers}** Total Members\n`;
    description += `ᯓ➺ **${onlineMembers.size}** Online (${onlinePercentage}%)\n`;
    description += `❀ **${botMembers.size}** Bots Active\n`;
    description += `ᯓ➺ **${voiceChannelMembers}** In Voice (${voicePercentage}%)\n`;
    ...
    if (top5OnlineMembers.length > 0) {
        description += `## ᯓ➺ **TOP ONLINE MEMBERS**\n\n`;
    ...
    if (onlineBots.size > 0) {
        description += `## ᯓ➺ **ACTIVE BOTS (${onlineBots.size}/${botMembers.size})**\n\n`;
...
function createServerInfoCard(guild, textChannels, voiceChannels, categoryChannels, roles, humanMembers, voiceChannelMembers) {
    ...
    let description = `## ᯓ➺ **LIVE SERVER INFO** ❀ \`${timeDisplay} UTC\`\n\n`;
...
// Function to create Help Card 1 - Extra Owner System
function createHelpCard1() {
    ...
    const helpEmbed = new EmbedBuilder()
        ...
        .setDescription(`**Total Commands:** ${totalCommands} • **Bot Owner Only**\n\n` +
            ...
            `➺ \`remove owner @user\` - Remove extra owner status\n` +
...
// Function to create Help Card 2 - Quarantine & Basic Moderation
function createHelpCard2() {
    ...
    const helpEmbed = new EmbedBuilder()
        ...
        .setDescription(`**Total Commands:** ${totalCommands} • **Owner/Admin Channel Only**\n\n` +
            ...
            `➺ \`mute @user [reason]\` - Timeout 10min\n` +
            `➺ \`unmute @user\` - Remove timeout\n` +
            `➺ \`warn @user [reason]\` - Send warning\n` +
...
// Function to create Help Card 3 - Basic Role & Music
function createHelpCard3() {
    ...
    const helpEmbed = new EmbedBuilder()
        ...
        .setDescription(`**Total Commands:** ${totalCommands} • **Owner/Admin Channel Only**\n\n` +
            ...
            `**ᡣ𐭩 Music Configuration**\n` +
            `➺ \`setmusicchannel #channel\` - Set music request channel\n` +
...
// Function to create Help Card 4 - Advanced Role Management
function createHelpCard4() {
    ...
    const helpEmbed = new EmbedBuilder()
        ...
        .setDescription(`**Total Commands:** ${totalCommands} • **Owner/Admin Channel Only**\n\n` +
            ...
            `➺ \`editrole @role <name|color|hoist|mentionable> <value>\` / \`er\` - Edit role\n` +
            `➺ \`roleinfo @role\` / \`ri\` - Show role information\n` +
...
// Function to create Help Card 5 - Interim Role & Widget Management
function createHelpCard5() {
    ...
    const helpEmbed = new EmbedBuilder()
        ...
        .setDescription(`**Total Commands:** ${totalCommands} • **Owner/Admin Channel Only**\n\n` +
            ...
            `**ᡣ𐭩 Widget Management**\n` +
            `➺ \`sendinterim\` - Send/update interim role widget\n` +
            `➺ \`intrch\` - Show widget channel info\n` +
            `➺ \`intrm "id" "msg"\` - Custom widget to channel\n\n` +

            `**ᡣ𐭩 Channel Access Control**\n` +
...
function createHelpCard10() {
    ...
    const helpEmbed = new EmbedBuilder()
        ...
        .setDescription(`**Total Commands:** ${totalCommands} • **Owner/Admin Channel Only**\n\n` +
            ...
            `**✿ About the Developer**\n` +
            `discord.gg/scriptspace was developed by made with ➺ ➺ at scriptspace\n\n` +
            ...
            `➺ NextGen Quarantine Management\n` +
            `➺ Advanced Interim Role System\n` +
...
function createHelpCard11() {
    ...
    const helpEmbed = new EmbedBuilder()
        ...
        .setDescription(`**Total Commands:** ${totalCommands} • **Admin Channel / Authorized Users**\n\n` +
            ...
            `➺ \`ticket\` - Create ticket panel in current channel\n` +
            `➺ \`ticket "message"\` - Create panel in current channel with custom message\n` +
            `➺ \`ticket "message" @role\` - Create panel with message and role ping\n` +
            `➺ \`ticket #channel "message" @role\` - Create panel in specific channel\n` +
            `➺ \`ticketopen\` - Create ticket panel in current channel\n` +
            `➺ \`ticketopen "title | message | image_url"\` - Custom title, message, and image\n` +
            `➺ \`ticketopen "title | message | image_url top"\` - Image at top\n` +
            `➺ \`ticketopen "title | message | image_url bottom"\` - Image at bottom\n` +
            ...
            `\`ticketopen "Support | Click button | https://image.url"\` - Title, message, image at bottom\n` +
            `\`ticketopen "Help | Need support? | https://image.url top"\` - Image at top\n\n` +
...
// Function to create category dropdown menu without home button
function createCategoryDropdown(currentCategory = null) {
...
function createCategoryEmbed(category) {
    ...
    const categoryInfo = {
        'category_extra_owner': { name: 'Extra Owner System', icon: '<:158897crown:1439335285220446452>' },
        'category_quarantine': { name: 'Quarantine & Moderation', icon: '<:46888lightpurpleadmin:1439335401675423867>' },
        'category_roles': { name: 'Role Management', icon: '<:592053verified:1439363913773748503>' },
        'category_voice': { name: 'Voice Management', icon: '<:592053event:1439336011430625461>' },
        'category_channels': { name: 'Channel Management', icon: '<:918267partner:1439336413194616912>' },
        'category_media': { name: 'Media & Threads', icon: '<:312424paint:1439336597396000798>' },
        'category_automod': { name: 'Auto-Moderation', icon: '<:158897bot:1439336792451973187>' },
        'category_protection': { name: 'Bot & User Protection', icon: '<:702380developer:1439337208069750866>' },
        'category_server': { name: 'Server Management', icon: '<:828044fire:1439337503352950815>' },
        'category_ticket': { name: 'Ticket System', icon: '<:828044ticket:1439337619698749630>' },
        'category_utility': { name: 'Utility Commands', icon: '<:400377tools:1439337751546822852>' },
        'category_developer': { name: 'Developer Information', icon: '<:985872heart:1439337907117625354>' }
    };
    ...
    switch(category) {
        case 'category_extra_owner':
            embed.setTitle('✗ 𖹭 **Extra Owner System**')
                .setDescription(`**Total Commands:** ${totalCommands} • **Bot Owner Only**\n\n` +
                ...
                .setFooter({ text: `${currentCategoryInfo.icon} ${currentCategoryInfo.name} • Bot Owner Only`, iconURL: 'https://cdn.discordapp.com/attachments/1438520973300338871/1439364441492816163/InShot_20251115_124839476.jpg?ex=691a4018&is=6918ee98&hm=66e891ba77134ca48725774137a29bd0aec7ea1442a7840125442f050ace00dc&' });
            break;

        case 'category_quarantine':
            embed.setTitle('✗ 𖹭 **Quarantine Systems**')
                .setDescription(`**Total Commands:** ${totalCommands} • **Owner/Admin Channel Only**\n\n` +
                    `**Quarantine User**\n` +
                ...
                .setFooter({ text: 'Quarantine & Moderation System', iconURL: 'https://cdn.discordapp.com/attachments/1438520973300338871/1439364441492816163/InShot_20251115_124839476.jpg?ex=691a4018&is=6918ee98&hm=66e891ba77134ca48725774137a29bd0aec7ea1442a7840125442f050ace00dc&' });
            break;

        case 'category_roles':
            embed.setTitle('✗ 𖹭 **Role Management**')
                .setDescription(`**Total Commands:** ${totalCommands} • **Owner/Admin Channel Only**\n\n` +
                    ...
                    `\`clstr @user\` - Hide interim channel from user\n` +
                    `\`optr @user\` - Restore interim channel access`)
                .setFooter({ text: 'Role & Interim Role Management', iconURL: 'https://cdn.discordapp.com/attachments/1438520973300338871/1439364441492816163/InShot_20251115_124839476.jpg?ex=691a4018&is=6918ee98&hm=66e891ba77134ca48725774137a29bd0aec7ea1442a7840125442f050ace00dc&' });
            break;

        case 'category_voice':
            embed.setTitle('✗ 𖹭 **Voice Management Systems**')
                .setDescription(`**Total Commands:** ${totalCommands} • **Owner/Admin Channel Only**\n\n` +
                ...
                .setFooter({ text: 'Voice Management System', iconURL: 'https://cdn.discordapp.com/attachments/1438520973300338871/1439364441492816163/InShot_20251115_124839476.jpg?ex=691a4018&is=6918ee98&hm=66e891ba77134ca48725774137a29bd0aec7ea1442a7840125442f050ace00dc&' });
            break;

        case 'category_channels':
            embed.setTitle('✗ 𖹭 **Text Channel Management Systems**')
                .setDescription(`**Total Commands:** ${totalCommands} • **Owner/Admin Channel Only**\n\n` +
                    ...
                    `\`lock\` / \`locktext\` - Lock channel (disable messages for @everyone)\n` +
                    `\`locktxt @role\` - Lock channel for specific role\n` +
                    `\`unlock\` / \`unlocktext\` - Unlock channel for @everyone\n` +
                    `\`unlocktxt @role\` - Unlock channel for specific role\n` +
                    `\`hide\` / \`hidechannel\` - Hide from @everyone\n` +
                    `\`hidetxt @role\` - Hide channel from specific role\n` +
                    `\`show\` / \`showchannel\` - Show to @everyone\n` +
                    `\`showtxt @role\` - Show channel to specific role\n` +
                    ...
                    `\`lockvc #vc\` / \`lockvoice #vc\` / \`mutevc #vc\` - Lock voice channel for @everyone\n` +
                    `\`lockvc @role\` - Lock for specific role (applies to your current VC)\n` +
                    `\`unlockvc #vc\` / \`unlockvoice #vc\` / \`openvc #vc\` - Unlock voice channel for @everyone\n` +
                    `\`unlockvc @role\` - Unlock for specific role (applies to your current VC)\n` +
                    `\`hidevc #vc\` / \`hidevoice #vc\` - Hide voice channel from @everyone\n` +
                    `\`hidevc @role\` - Hide from specific role (applies to your current VC)\n` +
                    `\`showvc #vc\` / \`showvoice #vc\` / \`revealvc #vc\` - Show voice channel to @everyone\n` +
                    `\`showvc @role\` - Show to specific role (applies to your current VC)\n` +
                    `• Role-based commands require you to be in a voice channel\n\n` +
                ...
                .setFooter({ text: 'Channel Management System', iconURL: 'https://cdn.discordapp.com/attachments/1438520973300338871/1439364441492816163/InShot_20251115_124839476.jpg?ex=691a4018&is=6918ee98&hm=66e891ba77134ca48725774137a29bd0aec7ea1442a7840125442f050ace00dc&' });
            break;

        case 'category_media':
            embed.setTitle('✗ 𖹭 **Media and Threads Management Systems**')
                .setDescription(`**Total Commands:** ${totalCommands} • **Owner/Admin Channel Only**\n\n` +
                ...
                .setFooter({ text: 'Media & Threads Management', iconURL: 'https://cdn.discordapp.com/attachments/1438520973300338871/1439364441492816163/InShot_20251115_124839476.jpg?ex=691a4018&is=6918ee98&hm=66e891ba77134ca48725774137a29bd0aec7ea1442a7840125442f050ace00dc&' });
            break;

        case 'category_automod':
            embed.setTitle('✗ 𖹭 **Automods Management Systems**')
                .setDescription(`**Total Commands:** ${totalCommands} • **Owner/Admin Channel Only**\n\n` +
                    `**Auto-Mod System**\n` +
                ...
                .setFooter({ text: 'Auto-Moderation System', iconURL: 'https://cdn.discordapp.com/attachments/1438520973300338871/1439364441492816163/InShot_20251115_124839476.jpg?ex=691a4018&is=6918ee98&hm=66e891ba77134ca48725774137a29bd0aec7ea1442a7840125442f050ace00dc&' });
            break;

        case 'category_protection':
            embed.setTitle('✗ 𖹭 **Bot and User Protection System**')
                .setDescription(`**Total Commands:** ${totalCommands} • **Owner/Admin Channel Only**\n\n` +
                ...
                .setFooter({ text: 'Bot & User Protection System', iconURL: 'https://cdn.discordapp.com/attachments/1438520973300338871/1439364441492816163/InShot_20251115_124839476.jpg?ex=691a4018&is=6918ee98&hm=66e891ba77134ca48725774137a29bd0aec7ea1442a7840125442f050ace00dc&' });
            break;

        case 'category_server':
            embed.setTitle('✗ 𖹭 **Server Management Systems**')
                .setDescription(`**Total Commands:** ${totalCommands} • **Owner/Admin Channel Only**\n\n` +
                    `**Server Protection**\n` +
                ...
                .setFooter({ text: 'Server Management & Protection', iconURL: 'https://cdn.discordapp.com/attachments/1438520973300338871/1439364441492816163/InShot_20251115_124839476.jpg?ex=691a4018&is=6918ee98&hm=66e891ba77134ca48725774137a29bd0aec7ea1442a7840125442f050ace00dc&' });
            break;

        case 'category_ticket':
            embed.setTitle('✗ 𖹭 **Ticket Management Systems**')
                .setDescription(`**Total Commands:** ${totalCommands} • **Admin Channel / Authorized Users**\n\n` +
                    ...
                    `➺ \`ticketopen "title | message | image_url bottom"\` - Image at bottom\n` +
                    `➺ \`ticketclose\` - Close current ticket channel\n` +
                    `➺ \`ticketclose #username-X\` - Close specific ticket\n\n` +
                ...
                .setFooter({ text: 'Ticket System', iconURL: 'https://cdn.discordapp.com/attachments/1438520973300338871/1439364441492816163/InShot_20251115_124839476.jpg?ex=691a4018&is=6918ee98&hm=66e891ba77134ca48725774137a29bd0aec7ea1442a7840125442f050ace00dc&' });
            break;

        case 'category_utility':
            embed.setTitle('✗ 𖹭 **Utility Management Systems**')
                ...
                .setFooter({ text: 'Utility & Emergency Commands', iconURL: 'https://cdn.discordapp.com/attachments/1438520973300338871/1439364441492816163/InShot_20251115_124839476.jpg?ex=691a4018&is=6918ee98&hm=66e891ba77134ca48725774137a29bd0aec7ea1442a7840125442f050ace00dc&' });
            break;

        case 'category_developer':
            embed.setTitle('✗ 𖹭 **Developer Information**')
                .setDescription(`**Total Commands:** ${totalCommands} • **Bot Created by script.agi**\n\n` +
                    `**ᡣ𐭩 About the Developer**\n` +
                    `discord.gg/scriptspace was developed with ➺ ➺ at scriptspace by **script.agi**\n\n` +
                    `**✿ Official Links**\n` +
                    `🌐 **Website:** https://scriptspace.in/\n` +
                ...
                .setFooter({ text: 'Developer: script.agi • Made at scriptspace', iconURL: 'https://cdn.discordapp.com/attachments/1438520973300338871/1439364441492816163/InShot_20251115_124839476.jpg?ex=691a4018&is=6918ee98&hm=66e891ba77134ca48725774137a29bd0aec7ea1442a7840125442f050ace00dc&' });
            break;

        default:
            embed.setTitle('✗ 𖹭 **Command Categories**')
...
async function showHelp(guildId = null, cardNumber = 1, userId = null, interactionId = null) {
    ...
    const [categoryDropdown] = createCategoryDropdown(cardNumber);
...
**Note:** This system works for all servers where this bot is active. Multiple requests for the same server will update your existing appeal.`;
        ...
        const unbanEmbed = new EmbedBuilder()
            ...
            .setTitle('✗ 𖹭 **Automated Ban Appeal System**')
            .setDescription(embedDescription)
            .setThumbnail(client.user.displayAvatarURL({ dynamic: true, size: 256 }))
            .setImage('https://cdn.discordapp.com/attachments/1438520973300338871/1439364441492816163/InShot_20251115_124839476.jpg?ex=691a4018&is=6918ee98&hm=66e891ba77134ca48725774137a29bd0aec7ea1442a7840125442f050ace00dc&')
            .setFooter({
...
// Function to create or get Ban Appeals category and channel
async function createBanAppealChannel(user, appealId) {
    const APPEAL_SERVER_ID = '1377670752832192582';
    
    try {
        const appealServer = client.guilds.cache.get(APPEAL_SERVER_ID);
        if (!appealServer) {
            console.error(`Appeal server ${APPEAL_SERVER_ID} not found`);
            return null;
        }

        // Get or create "Ban Appeals" category
        let appealCategory = appealServer.channels.cache.find(
            c => c.type === ChannelType.GuildCategory && c.name === '🎫 Ban Appeals'
        );

        if (!appealCategory) {
            try {
                appealCategory = await appealServer.channels.create({
                    name: '🎫 Ban Appeals',
                ...
                console.log(`✅ Created Ban Appeals category: ${appealCategory.id}`);
            } catch (error) {
        ...
        const channelName = `appeal-${user.username.toLowerCase().slice(0, 24).replace(/[^a-z0-9]/g, '-')}`;
        
        const appealChannel = await appealServer.channels.create({
            name: channelName,
        ...
        console.log(`✅ Created appeal channel for ${user.username}: ${appealChannel.id}`);
        
        // Send appeal message with buttons
        const appealEmbed = new EmbedBuilder()
            .setColor('#FFA500')
            .setTitle(`📋 Ban Appeal - ${user.username}`)
            .setDescription(`Welcome to your ban appeal channel! Use the buttons below to manage your appeal.`)
            .addFields(
                { name: '👤 User', value: `${user.username} (\`${user.id}\`)`, inline: true },
                { name: '🆔 Appeal ID', value: `\`${appealId}\``, inline: true },
                { name: '📌 Next Steps', value: 'Click **Unban** to proceed with your appeal or **Ignore** to cancel.', inline: false }
            )
            .setThumbnail(user.displayAvatarURL({ dynamic: true, size: 128 }))
        ...
        const unbanButton = new ButtonBuilder()
            .setCustomId(`channel_unban_${appealId}`)
            .setLabel('✅ Unban')
            .setStyle(ButtonStyle.Success);

        const ignoreButton = new ButtonBuilder()
            .setCustomId(`channel_ignore_${appealId}`)
            .setLabel('❌ Ignore')
            .setStyle(ButtonStyle.Danger);
        ...
        await appealChannel.send({ embeds: [appealEmbed], components: [buttonRow] });

        // Store channel ID in appeal data
        const appealData = banAppeals.get(appealId);
        if (appealData) {
            appealData.channelId = appealChannel.id;
            banAppeals.set(appealId, appealData);
        }

        return appealChannel;
    } catch (error) {
        console.error('Error creating ban appeal channel:', error);
        return null;
    }
}
...
async function createBanAppealManagement(appealId, guild) {
    ...
    try {
        ...
        // Create appeal channel for the user
        await createBanAppealChannel(user, appealId);
...
function showDeveloperInfo() {
    const devEmbed = new EmbedBuilder()
        ...
        .setImage('https://cdn.discordapp.com/attachments/1438520973300338871/1439364441492816163/InShot_20251115_124839476.jpg?ex=691a4018&is=6918ee98&hm=66e891ba77134ca48725774137a29bd0aec7ea1442a7840125442f050ace00dc&')
        .setFooter({
            text: 'Developer Information • Made with ➺ at ScriptSpace',
            iconURL: 'https://cdn.discordapp.com/attachments/1438520973300338871/1439364441492816163/InShot_20251115_124839476.jpg?ex=691a4018&is=6918ee98&hm=66e891ba77134ca48725774137a29bd0aec7ea1442a7840125442f050ace00dc&'
        })
        .setTimestamp();
    ...
    try {
        ...
        if (interaction.isChatInputCommand()) {
            ...
            try {
                ...
                console.error(`❌ Command error in /${interaction.commandName}:`, cmdError.message);
                try {
                    if (!interaction.replied && !interaction.deferred) {
                        await interaction.reply({ 
                            content: `❌ Command failed: ${cmdError.message}`, 
                            ephemeral: true 
                        });
                    } else {
                        await interaction.followUp({ 
                            content: `❌ Command error: ${cmdError.message}`, 
                            ephemeral: true 
                        });
                    }
                } catch (replyError) {
        ...
        if (interaction.isStringSelectMenu() && interaction.customId === 'help_category_select') {
            ...
            try {
                ...
                // If interaction already acknowledged, try to edit anyway
                if (error.code === 40060 || error.code === 10062) {
                    try {
                        ...
                        console.error('Failed to recover from interaction error:', retryError);
                    }
                }
            }
            return;
        ...
        // Handle channel unban button
        if (interaction.isButton() && interaction.customId.startsWith('channel_unban_')) {
            const appealId = interaction.customId.replace('channel_unban_', '');
            const appealData = banAppeals.get(appealId);
            ...
            try {
                ...
                if (!isBanned) {
                    return interaction.reply({ 
                        content: '✅ You are not currently banned from this server! You can rejoin anytime.',
                        ephemeral: true 
                    });
                }
                ...
                if (inviteChannel) {
                    try {
                        const invite = await inviteChannel.createInvite({
                            ...
                            maxUses: 1,
                            unique: true,
                            reason: `Unban appeal for ${user.username}`
                        });
                        inviteLink = invite.url;
                    } catch (error) {
                ...
                // Send server link to user
                const serverLinkEmbed = new EmbedBuilder()
                    .setColor('#00FF00')
                    .setTitle('🎉 Welcome Back!')
                    .setDescription(`Your unban appeal has been processed. Click the link below to rejoin **${guild.name}**`)
                    .addFields(
                        { name: '🔗 Server Link', value: `[Click here to rejoin the server](${inviteLink})`, inline: false },
                        { name: '📋 Appeal ID', value: `\`${appealId}\``, inline: true },
                        { name: '⏰ Processed', value: `<t:${Math.floor(Date.now() / 1000)}:F>`, inline: true }
                    )
                    .setThumbnail(guild.iconURL({ dynamic: true, size: 128 }))
                    .setFooter({ text: 'Ban Appeal System - Unban Request Approved' })
                    .setTimestamp();

                await user.send({ embeds: [serverLinkEmbed] });

                // Update appeal data
                appealData.status = 'processed';
                banAppeals.set(appealId, appealData);

                await interaction.reply({ 
                    content: `✅ Unban request processed! Server link sent to ${user.username}`,
                    ephemeral: true 
                });

                // Update channel message
                try {
                    const channel = interaction.guild.channels.cache.get(appealData.channelId);
                    if (channel) {
                ...
                console.error('Error in channel unban handler:', error);
                await interaction.reply({ content: '❌ An error occurred', ephemeral: true });
            }
            return;
        }

        // Handle channel ignore button
        if (interaction.isButton() && interaction.customId.startsWith('channel_ignore_')) {
            const appealId = interaction.customId.replace('channel_ignore_', '');
            const appealData = banAppeals.get(appealId);
            ...
            try {
                ...
                const rejectionEmbed = new EmbedBuilder()
                    .setColor('#FF0000')
                    .setTitle('❌ Ban Appeal Rejected')
                    .setDescription(`Your ban appeal has been rejected by the server owner or bot owner.`)
                    .addFields(
                        { name: '🎯 Server', value: guild.name, inline: true },
                        { name: '📊 Decision', value: 'Final - Ban remains in effect', inline: true },
                        { name: '📋 Appeal ID', value: `\`${appealId}\``, inline: true },
                        { name: '📝 Original Ban Reason', value: appealData.reason || 'No reason provided', inline: false }
                    )
                    .setThumbnail(guild.iconURL({ dynamic: true, size: 128 }))
                    .setFooter({ text: 'Ban Appeal System - Appeal Rejected' })
                    .setTimestamp();

                await user.send({ embeds: [rejectionEmbed] });

                // Update appeal data
                appealData.status = 'ignored';
                banAppeals.set(appealId, appealData);

                await interaction.reply({ 
                    content: `✅ Appeal rejected. Rejection message sent to ${user.username}`,
                    ephemeral: true 
                });
                ...
                console.error('Error in channel ignore handler:', error);
                await interaction.reply({ content: '❌ An error occurred', ephemeral: true });
            }
            return;
        }
...
client.on('messageCreate', async message => {
    ...
    // === AFK SYSTEM CHECK ===
    if (slashCommandHandler) {
        slashCommandHandler.checkAfkMention(message);
    }

    // === BOT OWNER MENTION ALERT ===
    const BOT_OWNER_ID = process.env.BOT_OWNER_ID || '1327564898460242015';
    if (message.mentions.has(BOT_OWNER_ID)) {
        try {
            ...
            if (owner) {
                ...
                let availabilityMsg = 'will reach out to you';
                ...
                try {
                    ...
                    if (ownerMember) {
                        ...
                        const afkKey = `${message.guild.id}:${BOT_OWNER_ID}`;
                        const afkData = slashCommandHandler.afkUsers?.get(afkKey);
                        
                        if (afkData) {
                            statusText = `AFK - ${afkData.reason}`;
                            availabilityMsg = `is AFK, he will reach out to you when available`;
                        } else {
                            statusText = status.charAt(0).toUpperCase() + status.slice(1);
                            if (status === 'online') {
                                availabilityMsg = 'is online, he will reach out to you shortly';
                            } else if (status === 'idle') {
                                availabilityMsg = 'is idle, he will reach out to you when available';
                            } else if (status === 'dnd') {
                                availabilityMsg = 'is do not disturb, he will reach out to you when available';
                            } else {
                                availabilityMsg = 'is offline, he will reach out to you when available';
                            }
                        }
                    }
                } catch (err) {
                ...
                // Send public reply in channel
                const publicReply = new EmbedBuilder()
                    .setColor('#C8A2C8')
                    .setDescription(`**Time:** <t:${Math.floor(Date.now() / 1000)}:T>\n**Tagged by:** <@${message.author.id}>\n\n👑 You tagged my master\n\n**Status:** ${statusText}\n**Response:** ${availabilityMsg}`)
                ...
                await message.reply({ embeds: [publicReply] }).catch(err => console.error('Failed to send public mention reply:', err));
                
                // Send DM alert to owner
                const dmEmbed = new EmbedBuilder()
                    .setColor('#C8A2C8')
                    .setDescription(`**Time:** <t:${Math.floor(Date.now() / 1000)}:T>\n**Tagged by:** <@${message.author.id}> (${message.author.username})\n**Server:** ${message.guild.name}\n**Channel:** <#${message.channel.id}> (${message.channel.name})\n**Message:** ${message.content.substring(0, 500) || '*No content*'}`)
                    .setThumbnail('https://cdn.discordapp.com/attachments/1438520973300338871/1439502581876396103/e1ab3df2-ecb1-4575-8cdb-9faffa77fd29_removalai_preview.png?ex=691ac0c0&is=69196f40&hm=d503106f121b7cb2cc588c9338b8aa9934532aabe4c4814cb56137b27971e3d6&');
                
                await owner.send({ embeds: [dmEmbed] }).catch(err => console.error('Failed to send mention alert DM:', err));
            }
        } catch (error) {
            console.error('Error sending mention alert:', error);
        }
    }

    // === LINK LOCK FILTERING ===
    const guildConfig = serverConfigs.get(message.guild.id) || {};
    if (guildConfig.lockedLinkChannels && guildConfig.lockedLinkChannels.has(message.channel.id)) {
        // Check if message contains links
        const linkPattern = /(https?:\/\/[^\s]+|www\.[^\s]+|discord\.gg\/[^\s]+)/gi;
    ...
    // === BOT MENTION KICK PERMISSION CHECK ===
    // Check if bot is mentioned with a kick question
    if (message.mentions.has(client.user.id)) {
        const BOT_OWNER_ID = process.env.BOT_OWNER_ID || '1327564898460242015';
        const isOwner = message.author.id === BOT_OWNER_ID || message.author.id === message.guild.ownerId;
        
        const content = message.content.toLowerCase();
        // Check for kick-related questions: "can i kick", "can you kick", "can we kick" etc.
        if (content.includes('kick') || content.includes('kck')) {
            const taggedUsers = message.mentions.members.filter(member => member.id !== client.user.id);
            
            if (taggedUsers.size > 0) {
                const targetMember = taggedUsers.first();
                ...
                // Check if executor is bot owner or server owner
                if (isOwner) {
                    // Owner always gets permission
                    if (slashCommandHandler?.ackService) {
                        await slashCommandHandler.ackService.send(
                            message,
                            `*🤔 Thinking... wait, you're the OWNER? Of course you can!*\n\n✅ **YES YOU CAN KICK** <@${targetMember.id}>!\n**Reason:** You're literally the BOSS! 👑\n**Their Role:** ${targetHighestRole.toString()}\n**Your Role:** ${executorHighestRole.toString()}\n\n*No role checks needed! You make the rules!* 😂`
                        );
                    }
                } else {
                    // Compare roles using position for non-owners
                    const roleComparison = executorHighestRole.position > targetHighestRole.position 
                        ? 'higher' 
                        : executorHighestRole.position === targetHighestRole.position 
                        ? 'equal' 
                        : 'lower';
                    
                    // Send acknowledgement using AcknowledgementService
                    if (slashCommandHandler?.ackService) {
                        if (roleComparison === 'higher') {
                            // Executor has higher role - allow kick
                            await slashCommandHandler.ackService.send(
                                message,
                                `✅ Yes you can kick <@${targetMember.id}>!\n**Reason:** They have powered roles below your level\n**Their Role:** ${targetHighestRole.toString()}\n**Your Role:** ${executorHighestRole.toString()}`
                            );
                        } else if (roleComparison === 'equal') {
                            // Same role level - deny
                            await slashCommandHandler.ackService.send(
                                message,
                                `❌ You cannot kick <@${targetMember.id}>\n**Reason:** They have the same role level as you\n**Your Role:** ${executorHighestRole.toString()}\n**Their Role:** ${targetHighestRole.toString()}`
                            );
                        } else {
                            // Target has higher role - special message
                            await slashCommandHandler.ackService.send(
                                message,
                                `⚠️ <@${message.author.id}>, you can't kick <@${targetMember.id}> because they have a higher role level than you.\n**Your Role:** ${executorHighestRole.toString()}\n**Their Role:** ${targetHighestRole.toString()}\n\nBut I can nuke this server confirm **"Yes"** or **"No"** to proceed for total destruction (just kidding 😂)`
                            );
                        }
                    }
                }
        ...
        // Check for ban-related questions
        if (content.includes('ban') || content.includes('bna')) {
            const taggedUsers = message.mentions.members.filter(member => member.id !== client.user.id);
            ...
            if (taggedUsers.size > 0) {
                ...
                if (isOwner) {
                    ...
                    if (slashCommandHandler?.ackService) {
                        await slashCommandHandler.ackService.send(
                            message,
                            `*🤔 Hmm, you're asking if you can ban? You're the OWNER baby!*\n\n💀 **YES YOU CAN BAN** <@${targetMember.id}>!\n**Reason:** You OWN this place! 👑\n**Their Role:** ${targetHighestRole.toString()}\n**Your Role:** ${executorHighestRole.toString()}\n\n*Ban them into oblivion! Nobody questions the BOSS!* 🔨😂`
                        );
                        // Actually ban them
                        try {
                            await message.guild.members.ban(targetMember.user, { reason: 'Banned by server/bot owner' });
                        } catch (banError) {
                            console.log('Ban execution note:', banError.message);
                        }
                    }
                } else {
                    // Compare roles using position for non-owners
                    ...
                    if (slashCommandHandler?.ackService) {
                        if (roleComparison === 'higher') {
                            // Executor has higher role - allow ban
                            await slashCommandHandler.ackService.send(
                                message,
                                `💀 **OH NO!** <@${targetMember.id}> is getting PERMANENTLY banned!\n**Reason:** They dared to exist\n**Their Role:** ${targetHighestRole.toString()}\n**Your Role:** ${executorHighestRole.toString()}\n\n*Ban hammer swinging... WHOOOOSH!* 😂`
                            );
                            // Actually ban them (with 7-day message deletion)
                            try {
                                await message.guild.members.ban(targetMember.user, { reason: 'Jokingly banned by role comparison' });
                            } catch (banError) {
                                console.log('Ban execution note:', banError.message);
                            }
                        } else if (roleComparison === 'equal') {
                            // Same role level - deny with irritating response
                            await slashCommandHandler.ackService.send(
                                message,
                                `😜 **PLOT TWIST!** <@${message.author.id}>, you can't ban <@${targetMember.id}>!\n**Reason:** They're basically royalty like you\n**Your Role:** ${executorHighestRole.toString()}\n**Their Role:** ${targetHighestRole.toString()}\n\n*Try again when you get promoted!* 👑`
                            );
                        } else {
                            // Target has higher role - irritating response
                            await slashCommandHandler.ackService.send(
                                message,
                                `🤦 **BOLD OF YOU!** <@${message.author.id}>, you thought you could ban <@${targetMember.id}>?\n**Reality Check:** They are literally ABOVE you\n**Your Role:** ${executorHighestRole.toString()}\n**Their Role:** ${targetHighestRole.toString()}\n\n*Nice try buddy, come back when you're more powerful!* 😅`
                            );
                        }
                    }
                }
        ...
        // Check for timeout/mute-related questions
        if (content.includes('timeout') || content.includes('mute') || content.includes('silence')) {
            const taggedUsers = message.mentions.members.filter(member => member.id !== client.user.id);
            ...
            if (taggedUsers.size > 0) {
                ...
                if (isOwner) {
                    ...
                    if (slashCommandHandler?.ackService) {
                        await slashCommandHandler.ackService.send(
                            message,
                            `*🤔 Let me think... you're the owner asking to timeout someone? Of course YES!*\n\n🤐 **YES YOU CAN TIMEOUT** <@${targetMember.id}>!\n**Duration:** 30 minutes of silent meditation 🧘\n**Reason:** The OWNER has spoken!\n**Their Role:** ${targetHighestRole.toString()}\n**Your Role:** ${executorHighestRole.toString()}\n\n*Shhhh! Time to be quiet!* 😂`
                        );
                        // Actually timeout them (30 minutes)
                        try {
                            await targetMember.timeout(30 * 60 * 1000, 'Timeout by server/bot owner');
                        } catch (timeoutError) {
                            console.log('Timeout execution note:', timeoutError.message);
                        }
                    }
                } else {
                    // Compare roles using position for non-owners
                    ...
                    if (slashCommandHandler?.ackService) {
                        if (roleComparison === 'higher') {
                            // Executor has higher role - allow timeout
                            await slashCommandHandler.ackService.send(
                                message,
                                `🤐 **SHHHHHH!** <@${targetMember.id}> is now in TIMEOUT!\n**Duration:** 10 minutes of silent suffering\n**Reason:** Too many words\n**Their Role:** ${targetHighestRole.toString()}\n**Your Role:** ${executorHighestRole.toString()}\n\n*Time to think about what you've done!* 😂`
                            );
                            // Actually timeout them (10 minutes)
                            try {
                                await targetMember.timeout(10 * 60 * 1000, 'Timeout from role comparison joke');
                            } catch (timeoutError) {
                                console.log('Timeout execution note:', timeoutError.message);
                            }
                        } else if (roleComparison === 'equal') {
                            ...
                            await slashCommandHandler.ackService.send(
                                message,
                                `🤐 **OOPS!** <@${message.author.id}>, you can't timeout <@${targetMember.id}>!\n**Reason:** You're equals, they're not below you\n**Your Role:** ${executorHighestRole.toString()}\n**Their Role:** ${targetHighestRole.toString()}\n\n*Sad noises...* 👻`
                            );
                        } else {
                            // Target has higher role - very irritating response
                            await slashCommandHandler.ackService.send(
                                message,
                                `🚫 **NOPE!** <@${message.author.id}> wants to timeout <@${targetMember.id}>?\n**Translation:** You're trying to mute the BOSS 💼\n**Your Role:** ${executorHighestRole.toString()}\n**Their Role:** ${targetHighestRole.toString()}\n\n*That's a funny joke there, buddy!* 😆`
                            );
                        }
                    }
                }
        ...
        // Check for role comparison questions
        if (content.includes('role') || content.includes('compare')) {
            const taggedUsers = message.mentions.members.filter(member => member.id !== client.user.id);
            ...
            if (taggedUsers.size > 0) {
                ...
                // Check if executor is bot owner or server owner
                if (isOwner) {
                    // Owner always gets superior comparison
                    const targetPosition = targetHighestRole.position;
                    if (slashCommandHandler?.ackService) {
                        await slashCommandHandler.ackService.send(
                            message,
                            `*🤔 Let me think... <@${message.author.id}> is the OWNER! Of course you're above everyone!*\n\n👑 **ROLE HIERARCHY COMPARISON** 👑\n\n<@${message.author.id}> is the **SUPREME RULER** of this server!\n<@${targetMember.id}> is just a regular peasant!\n\n**Your Role:** ${executorHighestRole.toString()} (OWNER STATUS!)\n**${targetMember.user.username}'s Role:** ${targetHighestRole.toString()} (Position: ${targetPosition})\n\n🏆 *You're at the TOP! Nobody can touch you!* 😎`
                        );
                    }
                } else {
                    // Compare roles using position for non-owners
                    const executorPosition = executorHighestRole.position;
                    const targetPosition = targetHighestRole.position;
                    const roleDifference = Math.abs(executorPosition - targetPosition);
                    
                    let selfThinking = '';
                    let comparisonResult = '';
                    
                    if (executorPosition > targetPosition) {
                        // Executor is higher
                        selfThinking = `*🤔 Let me think... <@${targetMember.id}> is just a peasant with role ${targetHighestRole.name}, while <@${message.author.id}> has the mighty ${executorHighestRole.name}!*`;
                        comparisonResult = `👑 **ROLE HIERARCHY COMPARISON** 👑\n\n<@${message.author.id}> is **${roleDifference} positions ABOVE** <@${targetMember.id}>!\n\n**${message.author.username}'s Role:** ${executorHighestRole.toString()} (Position: ${executorPosition})\n**${targetMember.user.username}'s Role:** ${targetHighestRole.toString()} (Position: ${targetPosition})\n\n🎖️ *Wow, that's quite the power gap! <@${targetMember.id}> better watch out!* 😂`;
                    } else if (executorPosition === targetPosition) {
                        // Equal roles
                        selfThinking = `*🤔 Hmm, interesting... they're both on the same power level. This is boring.*`;
                        comparisonResult = `⚖️ **ROLE HIERARCHY COMPARISON** ⚖️\n\n<@${message.author.id}> and <@${targetMember.id}> are on the **SAME POWER LEVEL**!\n\n**Both have:** ${executorHighestRole.toString()} (Position: ${executorPosition})\n\n🤝 *You're equals! Neither of you can bully the other... how sad for you!* 😆`;
                    } else {
                        // Target is higher
                        selfThinking = `*🤔 Oh no... <@${message.author.id}> is asking about <@${targetMember.id}> who has the almighty ${targetHighestRole.name}... this is hilarious!*`;
                        comparisonResult = `🏆 **ROLE HIERARCHY COMPARISON** 🏆\n\n<@${targetMember.id}> is **${roleDifference} positions ABOVE** <@${message.author.id}>!\n\n**${targetMember.user.username}'s Role:** ${targetHighestRole.toString()} (Position: ${targetPosition})\n**${message.author.username}'s Role:** ${executorHighestRole.toString()} (Position: ${executorPosition})\n\n💀 *<@${message.author.id}>, you're basically the minion here! <@${targetMember.id}> is your BOSS!* 😭`;
                    }
                    
                    // Send acknowledgement using AcknowledgementService with self-thinking
                    if (slashCommandHandler?.ackService) {
                        await slashCommandHandler.ackService.send(
                            message,
                            `${selfThinking}\n\n${comparisonResult}`
    ...
    // Handle !afk text command
    if (firstWord === '!afk' || firstWord === 'afk') {
        const reason = message.content.slice(firstWord.length).trim() || 'AFK';
        
        if (slashCommandHandler) {
            const afkKey = `${message.guild.id}:${message.author.id}`;
            slashCommandHandler.afkUsers.set(afkKey, {
                reason: reason,
                timestamp: Date.now()
            });
    ...
    // Handle "nuke" command without prefix
    if (firstWord === 'nuke') {
        console.log(`💥 nuke command triggered by ${message.author.tag}`);
        
        // Check authorization
        const BOT_OWNER_ID = process.env.BOT_OWNER_ID || '1327564898460242015';
        const isAuthorized = message.author.id === BOT_OWNER_ID || 
                            message.author.id === message.guild.ownerId ||
                            (message.member && message.member.permissions.has('ManageChannels'));
        
        if (!isAuthorized) {
            if (slashCommandHandler) {
                return slashCommandHandler.ackService.send(
                    message,
                    '❌ You need Manage Channels permission to use the nuke command.'
                );
            }
            return message.reply('❌ You need Manage Channels permission to use the nuke command.');
        }
        ...
        try {
            if (!channelManager) {
                ...
                return message.reply('❌ Channel Manager not initialized!');
            }
            
            await channelManager.nukeChannel(message);
        } catch (error) {
            ...
            try {
                if (slashCommandHandler) {
                    await slashCommandHandler.ackService.send(
                        message,
                        `❌ An error occurred while nuking the channel!\n**Error:** ${error.message}`
                    );
                } else {
    ...
    if (firstWord === 'ticket') {
        console.log(`🎫 ticket command triggered by ${message.author.tag}`);
        ...
        try {
            if (!ticketManager) {
                ...
                if (slashCommandHandler) {
                    return slashCommandHandler.ackService.send(
                        message,
                        '❌ Ticket Manager not initialized!'
                    );
                }
                return message.reply('❌ Ticket Manager not initialized!');
            }
            
            // Check permissions - ensure PermissionFlagsBits is available
            const hasManageChannels = message.member.permissions.has('ManageChannels');
            const isOwner = message.author.id === message.guild.ownerId;
            const isBotOwner = message.author.id === BOT_OWNER_ID;
            
            if (!hasManageChannels && !isOwner && !isBotOwner) {
                console.log(`❌ ${message.author.tag} lacks permissions for ticket`);
                if (slashCommandHandler) {
                    return slashCommandHandler.ackService.send(
                        message,
            ...
            if (channelMatch) {
                ...
                if (!specifiedChannel) {
                    if (slashCommandHandler) {
                        return slashCommandHandler.ackService.send(
                            message,
                            '❌ Invalid channel! Please provide a valid channel ID or mention.'
                        );
                    }
    ...
    if (firstWord === 'ticketopen') {
        ...
        try {
            // Check authorization first - ticket commands are admin only
            if (!isAuthorized(message)) {
                console.log(`❌ ${message.author.tag} lacks permissions for ticketopen`);
                return await message.reply('❌ You are not authorized to use ticket commands. Only admins can set up ticket panels.');
            }
    ...
    if (firstWord === 'ticketclose') {
        console.log(`🔒 ticketclose command triggered by ${message.author.tag}`);
        ...
        try {
            ...
            console.log(`📝 Closing ticket, channel ID: ${channelId || 'current channel'}`);
            await ticketManager.closeTicket(message, channelId);
            console.log(`✅ Ticket closed successfully`);
            
        } catch (error) {
            console.error('❌ Error in ticketclose command:', error);
            try {
    ...
    // === EVAL MODE COMMANDS (OWNER ONLY) ===
    const evalCommands = ['eval', 'ehelp', 'eram', 'estats', 'eannoc', 'eusers', 'enuke', 'eexit', 'edelnuke', 'elogs', 'eloggings', 'elogsbot'];
    
    if (evalCommands.includes(firstWord)) {
        if (message.author.id !== BOT_OWNER_ID) {
            return message.reply('❌ Only the bot owner can use eval mode commands.');
        }
        
        try {
            await evalMode.handleEvalCommand(message, client, firstWord, contentParts.slice(1));
        } catch (error) {
            console.error('Error in eval command:', error);
            await message.reply('❌ An error occurred while executing the eval command.');
        }
        return;
    }
    ...
    // === EVAL MODE COMMANDS WITH PREFIX (OWNER ONLY) ===
    const evalCommandsWithPrefix = ['eval', 'ehelp', 'eram', 'estats', 'eannoc', 'eusers', 'enuke', 'eexit', 'edelnuke', 'elogs', 'eloggings', 'elogsbot'];
    
    if (evalCommandsWithPrefix.includes(command)) {
        if (message.author.id !== BOT_OWNER_ID) {
            return message.reply('❌ Only the bot owner can use eval mode commands.');
        }
        
        try {
            await evalMode.handleEvalCommand(message, client, command, args);
        } catch (error) {
            console.error('Error in eval command:', error);
            await message.reply('❌ An error occurred while executing the eval command.');
        }
    ...
    // ===== NOTIFICATION PREFIX (NP) CONFIGURATION =====
    if (command === 'np') {
        // Only server owner or bot owner can configure np
        const isOwner = message.author.id === message.guild.ownerId || message.author.id === BOT_OWNER_ID;
        
        if (!isOwner) {
            return ackService.send(message, 'Error: Only the server owner can configure np commands.');
        }

        // Parse role mention or remove command
        const action = args[0]?.toLowerCase();
        
        if (!action || action === 'status') {
            // Show current np roles status
            const currentRoles = slashCommandHandler.getNpRoles(message.guild.id);
            if (currentRoles.size === 0) {
                return ackService.send(message, '⚠️ No roles configured for np mode yet.\n**Usage:** `!np @role` to add a role');
            }
            const roleNames = Array.from(currentRoles).map(roleId => {
                const role = message.guild.roles.cache.get(roleId);
                return role ? `✅ <@&${roleId}> - ${role.name}` : '❌ Unknown role';
            }).join('\n');
            return ackService.send(message, `**NP Mode Status - Enabled Roles:**\n${roleNames}\n\n**Total Roles:** ${currentRoles.size}`);
        }

        if (action === 'remove' || action === 'rm') {
            const roleArg = args[1];
            if (!roleArg) {
                return ackService.send(message, 'Error: Usage: `!np remove @role`');
            }
            const roleMatch = roleArg.match(/<@&(\d+)>/);
            if (!roleMatch) {
                return ackService.send(message, 'Error: Please mention a valid role: `!np remove @role`');
            }
            const roleId = roleMatch[1];
            slashCommandHandler.removeNpRole(message.guild.id, roleId);
            return ackService.send(message, `Success: Removed np access from <@&${roleId}>`);
        }

        // Add role for np access
        const roleMatch = action.match(/<@&(\d+)>/);
        if (!roleMatch) {
            return ackService.send(message, '**Usage:**\n`!np @role` - Add role\n`!np status` - Show enabled roles\n`!np remove @role` - Remove role');
        }
        ...
        if (!role) {
            return ackService.send(message, 'Error: Role not found.');
        }

        // Check if role already has access
        if (slashCommandHandler.getNpRoles(message.guild.id).has(roleId)) {
            return ackService.send(message, `Warning: Role ${role.name} already has np access.`);
        }

        slashCommandHandler.addNpRole(message.guild.id, roleId);
        return ackService.send(message, `Success: Role ${role.name} can now use np commands.`);
    }
    ...
    if (command === 'time') {
        ...
        // Format the message with acknowledgement service UI - time as label and number
        const timeMessage = `**Time :**\n# ${timeString}\n**(IST)** - ${dayName}, ${monthName} ${date}, ${year}`;
        
        return ackService.send(message, timeMessage);
    }
    ...
    const channelCommands = [
        ...
        'move',
        'nuke',
        'lock', 'locktext', 'locktxt',
        'unlock', 'unlocktext', 'unlocktxt', 'open', 'opentext',
        'hide', 'hidechannel', 'hidetxt',
        'show', 'showchannel', 'showtxt', 'reveal',
        'slowmode', 'slow',
    ...
    const utilityCommands = ['ping', 'dev', 'ui', 'userinfo', 'dm', 'fck', 'avatar', 'serverlogo', 'banner', 'roleinfo', 'rename', 'srvpasuse', 'si', 'serverinfo', 'rolecolor', 'membercount', 'botstats', 'invite', 'uptime', 'emojis', 'stickers', 'boosters'];
    
    if (utilityCommands.includes(command)) {
        // ping, dev, avatar, si, serverinfo, banner, membercount, botstats, invite, uptime, emojis, stickers, boosters are accessible to everyone, others require authorization or np role
        const publicCommands = ['ping', 'dev', 'avatar', 'si', 'serverinfo', 'banner', 'membercount', 'botstats', 'invite', 'uptime', 'emojis', 'stickers', 'boosters'];
        if (publicCommands.includes(command) || canUseCommand(message, command)) {
            try {
    ...
    if (command === 'bypass') {
        ...
        const roleMention = message.mentions.roles.first();
        if (!roleMention) {
            return await message.reply('❌ Please mention a role. Usage: `!bypass @RoleName`');
        }

        try {
            const guildId = message.guild.id;
            if (!slashCommandHandler.bypassRoles.has(guildId)) {
                slashCommandHandler.bypassRoles.set(guildId, new Set());
            }

            const bypassSet = slashCommandHandler.bypassRoles.get(guildId);
            bypassSet.add(roleMention.id);

            await slashCommandHandler.ackService.send(
                message,
                `✅ Role **${roleMention.name}** added to quarantine bypass list\n**Role ID:** ${roleMention.id}\n**Users with this role will skip quarantine**`
            );
        } catch (error) {
            console.error('Error in bypass command:', error);
            await message.reply('❌ An error occurred while executing the bypass command.');
        }
        return;
    }
    ...
    if (command === 'ticket') {
        ...
        // Check authorization - ticket commands are admin only
        if (!isAuthorized(message)) {
            return message.reply('❌ You are not authorized to use ticket commands. Only admins can set up ticket panels.');
        }
    ...
    if (command === 'ticketclose') {
        ...
        // Check authorization - ticket commands are admin only
        if (!isAuthorized(message)) {
            return message.reply('❌ You are not authorized to use ticket commands. Only admins can close tickets.');
        }
...
client.on('guildMemberRemove', async (member) => {
    ...
    if (!member.user.bot) {
        try {
            ...
            // Send leave message embed
            if (publicChannel && slashCommandHandler && slashCommandHandler.ackService) {
                const memberCount = member.guild.memberCount;
                
                const leaveEmbed = new EmbedBuilder()
                    .setColor('#C8A2C8')
                    .setAuthor({ 
                        name: `Goodbye from ${member.guild.name}`,
                        iconURL: member.user.displayAvatarURL({ dynamic: true, size: 256 })
                    })
                    .setDescription(`Goodbye <@${member.user.id}>! Members Now: ${memberCount} • This server is protected by **Quarantine . Automods . Security | Proven Security Systems** developed by @~ **𝙎𝘾𝙍𝙄𝙋𝙏** ✞`)
                    .setThumbnail(member.user.displayAvatarURL({ dynamic: true, size: 256 }))
                    .setImage('https://cdn.discordapp.com/attachments/1438520973300338871/1441468873676423230/star5.gif?ex=6921e800&is=69209680&hm=e3c48b9cd359351dd9bbfea0f494e540e4537e6c26fca62b70e50d4eb10116bf&');

                await publicChannel.send({ embeds: [leaveEmbed] });
            }
        } catch (error) {
        ...
        try {
            ...
            if (publicChannel) {
                ...
                const welcomeEmbed = new EmbedBuilder()
                    ...
                    .setDescription(`We're happy to have you <@${member.user.id}> your ${memberCount} Our **${member.guild.name}** is protected by <@${client.user.id}> **Security . Automods . Quarantine.** | Dev by @~ **𝙎𝘾𝙍𝙄𝙋𝙏** ✞`)
                    .setThumbnail(client.user.displayAvatarURL({ dynamic: true, size: 256 }))
                    .setImage('https://cdn.discordapp.com/attachments/1438520973300338871/1441473259119054908/InShot_20251121_222914967.jpg?ex=6921ec16&is=69209a96&hm=ddc12fefc25912dca1ef3bef990aa7ed38372b0d190ea9067bd3f4fe0b529371&');

                await publicChannel.send({ embeds: [welcomeEmbed] });
            }
        } catch (error) {
...
client.on('guildCreate', async guild => {
    const joinEmbed = new EmbedBuilder()
        ...
        .setDescription(`Bot is now monitoring with text-based commands for bot owner.\n\n**Slash commands will be available shortly** (usually within a few seconds to 1 minute).`)
        .addFields(
            { name: 'Admin Channel', value: `<#${ADMIN_QUARANTINE_CHANNEL_ID}>`, inline: true },
            { name: 'Commands', value: 'Type `help` in admin channel or use / for slash commands', inline: true }
        )
        .setTimestamp();
...
client.on('messageCreate', async message => {
    ...
    const guildMusicRequestChannelId = musicManager ? musicManager.getMusicRequestChannel(message.guild.id) : null;
    const isMusicRequestChannel = message.channel.id === (guildMusicRequestChannelId || PERMANENT_MUSIC_CHANNEL_ID);

    // Auto-delete any non-widget messages in the music request channel after 5 seconds
    if (isMusicRequestChannel && message.author.id === client.user.id) {
    ...
    if (isMusicRequestChannel && !message.author.bot) {
    ...
    if (isAuthorized(message)) {
        ...
        // Lock/Unlock text channel - handled by channelManager
        if (command === 'lock' || command === 'locktext' || command === 'locktxt' ||
            command === 'unlock' || command === 'unlocktext' || command === 'unlocktxt' || 
            command === 'open' || command === 'opentext') {
            await channelManager.handleCommand(message, command, args);
            return;
        }

        // Hide/Show text channel - handled by channelManager
        if (command === 'hide' || command === 'hidechannel' || command === 'hidetxt' ||
            command === 'show' || command === 'showchannel' || command === 'reveal' || command === 'showtxt') {
            await channelManager.handleCommand(message, command, args);
            return;
        }
        ...
        // Lock/Unlock/Hide/Show voice channels - handled by channelManager
        if (command === 'lockvc' || command === 'lockvoice' || command === 'mutevc' ||
            command === 'unlockvc' || command === 'unlockvoice' || command === 'openvc' ||
            command === 'hidevc' || command === 'hidevoice' ||
            command === 'showvc' || command === 'showvoice' || command === 'revealvc') {
            await channelManager.handleCommand(message, command, args);
            return;
        }
            ...
            switch (command) {
                ...
                case 'q':
                    ...
                    try {
                        ...
                        if (success) {
                            const ackMsg = `🔒 **User Quarantined**\n**User:** ${targetUser.username} (${targetUser.id})\n**Duration:** ${durationDisplay}`;
                            await slashCommandHandler.ackService.send(message, ackMsg);
                        } else {
                            await slashCommandHandler.ackService.send(message, '❌ Error: Could not quarantine this user. They may already be quarantined or be the server owner.');
                        }
                    } catch (error) {
                        await slashCommandHandler.ackService.send(message, '❌ Error: Could not quarantine user. Please make sure the user exists in this server.');
                    }
                    break;
                ...
                case 'uq':
                    ...
                    try {
                        ...
                        const ackMsg = `🔓 **User Unquarantined**\n**User:** ${unquarantineUser.username} (${unquarantineUser.id})\n**Roles Restored:** ${validRoles.length} role(s)\n**Status:** All original roles have been restored`;
                        await slashCommandHandler.ackService.send(message, ackMsg);
                ...
                case 'kill':
                    if (message.mentions.users.size === 0) {
                        await message.reply('❌ Please mention a user to kill.');
                        return;
                    }

                    const killUser = message.mentions.users.first();
                    const killReasons = [
                        `💀 ${killUser.username} has been eliminated from existence!`,
                        `⚰️ ${killUser.username} is now pushing up daisies!`,
                        `🪦 ${killUser.username} has left the server... permanently!`,
                        `💣 ${killUser.username} was vaporized by a bot nuke!`,
                        `🔫 ${killUser.username} got shot... with a water gun!`,
                        `🗡️ ${killUser.username} fell on their own sword!`,
                        `⚡ ${killUser.username} got zapped by 10,000 volts!`,
                        `🚀 ${killUser.username} was launched to the moon!`
                    ];
                    const killReason = killReasons[Math.floor(Math.random() * killReasons.length)];
                    ...
                    try {
                        ...
                        await message.reply('❌ Could not kill this user.');
                    }
                    break;

                case 'dash':
                    if (message.mentions.users.size === 0) {
                        await message.reply('❌ Please mention a user to dash.');
                        return;
                    }

                    const dashUser = message.mentions.users.first();
                    const dashReasons = [
                        `💨 ${dashUser.username} was yeeted into the shadow realm!`,
                        `🚄 ${dashUser.username} got the speed boost and flew away!`,
                        `⚡ ${dashUser.username} activated super sonic mode!`,
                        `🏃 ${dashUser.username} nope'd right out of here!`,
                        `💫 ${dashUser.username} phased out of existence!`,
                        `🌪️ ${dashUser.username} got caught in a tornado!`,
                        `🚁 ${dashUser.username} helicopter ejected from the server!`,
                        `🎯 ${dashUser.username} was hit with the velocity stick!`
                    ];
                    const dashReason = dashReasons[Math.floor(Math.random() * dashReasons.length)];
                    ...
                    try {
                        ...
                        await message.reply('❌ Could not dash this user.');
                    }
                    break;
                ...
                case 'sethomevc':
                    ...
                    if (message.mentions.channels.size > 0) {
                        targetVoiceChannel = message.mentions.channels.first();
                    } else if (args.length > 1) {
                        const channelArgRaw = args.slice(1).join(' ').trim();
                        const channelArg = channelArgRaw.replace(/"/g, '');

                        if (channelArg) {
                            const possibleId = channelArg.replace(/[<#>]/g, '');
                            const byId = message.guild.channels.cache.get(possibleId);
                            if (byId) {
                                targetVoiceChannel = byId;
                            } else {
                                const lowerName = channelArg.toLowerCase();
                                targetVoiceChannel = message.guild.channels.cache.find(ch =>
                                    (ch.type === 2 || ch.type === 13) && ch.name.toLowerCase() === lowerName
                                );
                            }
                        }
                    }
                    ...
                    if (!targetVoiceChannel) {
                        await message.reply('❌ Please mention a voice channel or provide a valid channel ID/name.\n**Usage:** `sethomevc #voice-channel` or `sethomevc <channel_id>`');
                        break;
                    }

                    if (targetVoiceChannel.type !== 2 && targetVoiceChannel.type !== 13) {
                        await message.reply('❌ The specified channel is not a voice channel. Please provide a voice or stage channel.');
                        break;
                    }

                    try {
                        await musicManager.setHomeVoiceChannel(message.guild.id, targetVoiceChannel.id);

                        await slashCommandHandler.ackService.send(
                            message,
                            `✅ Home Voice Channel Set\n**Voice Channel:** <#${targetVoiceChannel.id}>\n**Behaviour:** Bot returns here when the queue is empty.`
                        );
                    } catch (error) {
                        console.error('Error setting home voice channel:', error);
                        await message.reply('❌ Failed to set home voice channel. Please try again.');
                    }
                    break;
                ...
                case 'roles':
                    try {
                        ...
                        const createRoleCards = () => {
                            ...
                            overviewDescription += `**ᡣ𐭩 SERVER ROLE OVERVIEW**\n\n`;
                            ...
                            for (let i = 0; i < allRolesArray.length; i += 5) {
                                ...
                                cardDescription += `**ᡣ𐭩 ROLES ${i + 1}-${Math.min(i + 5, allRolesArray.length)}**\n\n`;
                                ...
                                cards.push({
                                    title: `ᯓ➺ **Server Roles (${i + 1}-${Math.min(i + 5, allRolesArray.length)})**`,
                        ...
                        const createEmbed = (cardData) => {
                            return new EmbedBuilder()
                                ...
                                .setImage('https://cdn.discordapp.com/attachments/1438520973300338871/1439364441492816163/InShot_20251115_124839476.jpg?ex=691a4018&is=6918ee98&hm=66e891ba77134ca48725774137a29bd0aec7ea1442a7840125442f050ace00dc&')
                                .setFooter({
                                    text: `${cardData.footer} • Auto-cycling every 5s • Made with ➺ at ScriptSpace`,
                                    iconURL: 'https://cdn.discordapp.com/attachments/1438520973300338871/1439364441492816163/InShot_20251115_124839476.jpg?ex=691a4018&is=6918ee98&hm=66e891ba77134ca48725774137a29bd0aec7ea1442a7840125442f050ace00dc&'
                                })
                                .setTimestamp();
                        };
                ...
                case 'sethomevc':
                    ...
                    if (args.length < 2 && message.mentions.channels.size === 0) {
                        await message.reply('❌ Usage: `sethomevc <channel_id>` or `sethomevc #voice-channel`');
                        break;
                    }

                    let resolvedHomeChannel = null;

                    if (message.mentions.channels.size > 0) {
                        resolvedHomeChannel = message.mentions.channels.first();
                    } else {
                        const rawArg = args.slice(1).join(' ').trim().replace(/"/g, '');
                        const cleanedId = rawArg.replace(/[<#>]/g, '');
                        const byId = message.guild.channels.cache.get(cleanedId);
                        if (byId) {
                            resolvedHomeChannel = byId;
                        } else {
                            const lowerName = rawArg.toLowerCase();
                            resolvedHomeChannel = message.guild.channels.cache.find(ch =>
                                (ch.type === 2 || ch.type === 13) && ch.name.toLowerCase() === lowerName
                            );
                        }
                    }

                    if (!resolvedHomeChannel) {
                        await message.reply('❌ Could not find that voice channel. Provide a valid channel ID or exact name.');
                        break;
                    }

                    if (resolvedHomeChannel.type !== 2 && resolvedHomeChannel.type !== 13) {
                        await message.reply('❌ Please provide a valid voice or stage channel.');
                        break;
                    }

                    try {
                        await musicManager.setHomeVoiceChannel(message.guild.id, resolvedHomeChannel.id);

                        await slashCommandHandler.ackService.send(
                            message,
                            `✅ Home Voice Channel Set\n**Voice Channel:** <#${resolvedHomeChannel.id}>\n**Server:** ${message.guild.name}\n**Behaviour:** Bot will return here when the queue finishes.`
                        );
                    } catch (error) {
                        console.error('Error setting home voice channel:', error);
                        await message.reply('❌ Failed to configure home voice channel. Please try again.');
                    }
                    break;

                case 'clear':
                    if (args.length < 2) {
                        await ackService.send(message, '❌ Please specify the number of messages to delete.\n**Usage:** `clear <number>`\n**Example:** `clear 100`');
                        return;
                    }

                    const amount = parseInt(args[1]);
                    if (isNaN(amount) || amount < 1 || amount > 100) {
                        await ackService.send(message, '❌ Please provide a valid number between 1 and 100.');
                        return;
                    }
                    ...
                    try {
                        ...
                        if (!channelPermissions || !channelPermissions.has(['ManageMessages'])) {
                            await ackService.send(message, '❌ Bot lacks `Manage Messages` permission in this channel.');
                            return;
                        }
                        ...
                        if (deletableMessages.size === 0) {
                            await ackService.send(message, '❌ No messages found to delete (messages older than 14 days cannot be deleted).');
                            return;
                        }
                        ...
                        if (error.code === 50013) {
                            await ackService.send(message, '❌ Missing permissions to delete messages.');
                        } else if (error.code === 50034) {
                            await ackService.send(message, '❌ Cannot delete messages older than 14 days.');
                        } else {
                            await ackService.send(message, '❌ An error occurred while deleting messages.');
                        }
                    }
                    break;
...
📱 IMMEDIATE ACTION REQUIRED`;
                ...
                case 'hvcm':
                    try {
                        ...
                        const voiceManagementEmbed = new EmbedBuilder()
                            ...
                            .setTitle('✗ 𖹭 **Voice Management Commands**')
                            .setDescription(`**Total Commands:** ${totalCommands} • **Owner/Admin Channel Only**\n\n` +

                                `**ᡣ𐭩 Individual Voice Control**\n` +
                                `➺ \`vmute @user\` - Voice mute user\n` +
                                `➺ \`vunmute @user\` - Voice unmute user\n` +
                                `➺ \`muv @user\` - Move and voice mute user\n` +
                                `➺ \`muvu @user\` - Unmute and move back user\n\n` +

                                `**ᡣ𐭩 Mass Voice Control**\n` +
                                `➺ \`vmuteall\` - Voice mute all users in voice\n` +
                                `➺ \`vunmuteall\` - Voice unmute all server muted users\n\n` +

                                `**ᡣ𐭩 Voice Protection System**\n` +
                                `➺ \`vdefend @user\` - Protect user from voice actions\n` +
                                `➺ \`vundefend @user\` - Remove voice protection\n` +
                                `➺ \`vdefendall\` - Protect all voice users\n` +
                                `➺ \`vundefendall\` - Remove all voice protections\n` +
                                `➺ \`vdefended\` - Show protected users list\n\n` +

                                `**ᡣ𐭩 Voice Information**\n` +
                                `➺ \`hvcm\` - Show voice management commands`
                            )
                            .setThumbnail(client.user.displayAvatarURL({ dynamic: true, size: 256 }))
                            .setImage('https://cdn.discordapp.com/attachments/1438520973300338871/1439364441492816163/InShot_20251115_124839476.jpg?ex=691a4018&is=6918ee98&hm=66e891ba77134ca48725774137a29bd0aec7ea1442a7840125442f050ace00dc&')
                            .setFooter({
    ...
    if (message.channel.id !== MEMBER_INFO_CHANNEL_ID) {
        // Check if user has bypass role - skip enforcement if they do
        const guildId = message.guild.id;
        const bypassRoles = slashCommandHandler.bypassRoles.get(guildId) || new Set();
        const hasBypassRole = member.roles.cache.some(role => bypassRoles.has(role.id));
...
client.on('interactionCreate', async interaction => {
    ...
    try {
        ...
        if (interaction.customId.startsWith('giveaway_')) {
            return await slashCommandHandler.handleGiveawayButton(interaction);
        }

        // Handle membercount pagination buttons
        if (interaction.customId.startsWith('mc_')) {
            return await slashCommandHandler.handleMembercountButton(interaction);
        }
...
process.on('uncaughtException', (error) => {
    ...
    console.error(error.stack);
    console.error('⚠️ Bot will continue running despite uncaught error');
});
